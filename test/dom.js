...
QUnit.test('object creation tests', function (assert) {
    ...
    assert.equal(
        ...
        wrap($('<div id="test" class="cls"></div>').get(0)),
    );
    ...
    assert.equal(
        ...
        wrap($('<div id="test" class="cls" customAttr="custom"></div>').get(0)),
    );
...
QUnit.test('string creation tests', function (assert) {
    assert.equal(
        wrap(this.generator('div').$fragment),
        $('<div><div></div></div>').html(),
    );
    assert.equal(
        wrap(this.generator('div div').$fragment),
        $('<div><div></div><div></div></div>').html(),
    );
    assert.equal(
        wrap(this.generator('div div..div').$fragment),
        $('<div><div></div><div><div></div></div></div>').html(),
    );
    assert.equal(
        wrap(this.generator('div div..div..div').$fragment),
        $('<div><div></div><div><div><div></div></div></div></div>').html(),
    );
    assert.equal(
        wrap(this.generator('div div div..div..div').$fragment),
        $(
            '<div><div></div><div></div><div><div><div></div></div></div></div>',
        ).html(),
    );
    assert.equal(
        wrap(this.generator('div div..div div..div..div').$fragment),
        $(
            '<div><div></div><div><div></div></div><div><div><div></div></div></div></div>',
        ).html(),
    );
    assert.equal(
        wrap(this.generator('div div..div..div div..div..div').$fragment),
        $(
    ...
    assert.equal(
        wrap(
            this.generator('div div..div..div(class=test) div..div..div').$fragment,
        ),
        $(
    ...
    assert.equal(
        wrap(
            this.generator('div div..div..div(class=test) div(id=myId)..div..div')
                .$fragment,
        ),
        $(
    ...
    assert.equal(
        wrap(this.generator('div(id=test)').$fragment),
        $('<div><div id="test"></div></div>').html(),
    );
    assert.equal(
        wrap(this.generator('div(class=test)').$fragment),
        $('<div><div class="test"></div></div>').html(),
    );
    assert.equal(
        wrap(this.generator('div(class=test,customAttr=custom)').$fragment),
        $('<div><div class="test" customAttr="custom"></div></div>').html(),
    );
    assert.equal(
        wrap(this.generator('img(src=http://example.com/test.gif)').$fragment),
...
QUnit.test('getString() tests', function (assert) {
    ...
    assert.equal(
        this.generator('div div..div..div(class=test) div..div..div').getString(),
        $(
            '<div><div></div><div><div><div class="test"></div></div></div><div><div><div></div></div></div></div>',
        ).html(),
    );
...
QUnit.test('selecting elements tests', (assert) => {
    const generator = new Generatorjs(
        {
            el: 'div',
            attr: 'id=div1,class=cls',
    ...
    const t1 = walkThrough(
        nodeListToArray(generator.get('.c').$selected),
        nodeListToArray($($el).find('.c').get()),
        assert,
    );
